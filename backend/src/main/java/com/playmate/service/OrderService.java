...
public class OrderService {
    ...
    public OrderResponse createOrder(Long userId, CreateOrderRequest request) {
        ...
        // 验证价格
        BigDecimal expectedAmount = player.getPrice().multiply(BigDecimal.valueOf(request.getDuration() / 60.0));
    ...
    private OrderResponse convertToResponse(Order order) {
        ...
        playerInfo.setUsername(order.getPlayer().getUsername());
        playerInfo.setAvatar(order.getPlayer().getAvatar());
        playerInfo.setGame(order.getPlayer().getGame());
        playerInfo.setPrice(order.getPlayer().getPrice());
