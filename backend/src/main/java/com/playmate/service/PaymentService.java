...
public class PaymentService {
    ...
    @Transactional
    public Payment createRechargePayment(@NonNull Long userId, @NonNull BigDecimal amount, @NonNull Payment.PaymentMethod paymentMethod) {
    ...
    @Transactional
    public Payment createOrderPayment(@NonNull Long userId, Long orderId, @NonNull BigDecimal amount, @NonNull Payment.PaymentMethod paymentMethod) {
        User user = userRepository.findById(userId)
            .orElseThrow(() -> new RuntimeException("用户不存在"));
    ...
    public Payment processPayment(@NonNull Long paymentId, @NonNull String transactionId) {
        ...
        // if (paymentRepository.existsByTransactionIdAndStatus(transactionId, Payment.PaymentStatus.SUCCESS)) {
