...
class WithdrawalProvider with ChangeNotifier {
  ...
  List<WithdrawalApplication> get withdrawalApplications =>
  ...
  Future<bool> createWithdrawalApplication({
    ...
    try {
      _currentApplication = await _withdrawalService
  ...
  Future<bool> getWithdrawalApplicationDetail(String applicationId) async {
    ...
    try {
      _currentApplication = await _withdrawalService
          .getWithdrawalApplicationDetail(applicationId);

      // 更新列表中的申请
      final index = _withdrawalApplications.indexWhere(
  ...
  Future<bool> cancelWithdrawalApplication(String applicationId) async {
    ...
    try {
      final success = await _withdrawalService.cancelWithdrawalApplication(
        applicationId,
      );
      ...
      if (success) {
        ...
        if (index != -1) {
          _withdrawalApplications[index] = _withdrawalApplications[index]
  ...
  Future<bool> deleteWithdrawalAccount(String accountId) async {
    ...
    try {
      final success = await _withdrawalService.deleteWithdrawalAccount(
        accountId,
      );
