...
class PostService {
  ...
  /// 获取待审核动态（管理员）
  static Future<List<Post>> getPendingPosts({
    int page = 0,
    int size = 20,
  }) async {
    try {
      final response = await _dio.get('/posts/admin/pending', queryParameters: {
        'page': page,
        'size': size,
      });
      ...
      if (response.data['success'] == true) {
        ...
        throw Exception('获取待审核动态失败: ${response.data['message']}');
      }
    } on DioException catch (e) {
      throw Exception('网络错误: ${e.message}');
    }
  ...
  /// 获取置顶动态
  static Future<List<Post>> getPinnedPosts({
    int page = 0,
    int size = 20,
  }) async {
    try {
      final response = await _dio.get('/posts/pinned', queryParameters: {
        'page': page,
        'size': size,
      });
      ...
      if (response.data['success'] == true) {
        ...
        throw Exception('获取置顶动态失败: ${response.data['message']}');
      }
    } on DioException catch (e) {
      throw Exception('网络错误: ${e.message}');
    }
  ...
  /// 审核动态（管理员）
  static Future<void> approvePost(String postId, bool approved) async {
    try {
      await _dio.put('/posts/admin/$postId/approve', data: {
        'approved': approved,
      });
    } on DioException catch (e) {
      throw Exception('审核动态失败: ${e.message}');
    }
  }
  ...
  static Future<void> rejectPost(String postId) async {
    try {
      await _dio.put('/posts/admin/$postId/reject');
    } on DioException catch (e) {
      throw Exception('拒绝动态失败: ${e.message}');
    }
  }
  ...
  static Future<void> unpinPost(String postId) async {
    try {
      await _dio.put('/posts/admin/$postId/unpin');
    } on DioException catch (e) {
      throw Exception('取消置顶失败: ${e.message}');
    }
  }

  /// 删除动态（管理员）
  static Future<void> deletePostAdmin(String postId) async {
    try {
      await _dio.delete('/posts/admin/$postId');
    } on DioException catch (e) {
      throw Exception('删除动态失败: ${e.message}');
    }
  }
}